...
func hget(client *redis.Client) {
	...
	title, err := client.Cmd("HGET", "album:1", "title").Str()
	if err != nil {
		log.Fatal(err)
	}
	// Similarly, get the artist and convert it to a string.
	artist, err := client.Cmd("HGET", "album:1", "artist").Str()
	if err != nil {
		log.Fatal(err)
	}
	// And the price as a float64...
	price, err := client.Cmd("HGET", "album:1", "price").Float64()
	if err != nil {
		log.Fatal(err)
	}
	// And the number of likes as an integer.
	likes, err := client.Cmd("HGET", "album:1", "likes").Int()
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("%s by %s: £%.2f [%d likes]\n", title, artist, price, likes)
}
func hmget(client *redis.Client) {
	resp := client.Cmd("HMGET", "album:1", "title", "artist", "price", "likes")
	fmt.Println(resp.String())
}
func hmset(client *redis.Client) {
	...
	resp := client.Cmd("HMSET", "album:1", "title", "Electric Ladyland", "artist", "Jimi Hendrix", "price", 4.95, "likes", 8)
